...
(() => {
  ...
  // Release clauses: every player in Spain has one (it is the law there); elsewhere about a quarter of contracts do.
  function genClause(p, leagueId) {
    const spain = leagueId === 'ESP' || leagueId === 'ESP2';
    if (!spain && rand() > 0.25) return 0;
    const m = spain ? (p.ovr >= 82 ? 3 + rand() * 3 : 1.8 + rand() * 1.7) : 1.4 + rand() * 1.4;
    return niceRound(playerValue(p) * m);
  }
  function assignClauses(W) {
    for (const p of Object.values(W.players)) {
      if (p.relc != null) continue;
      const cl = W.clubs[p.clubId];
      p.relc = cl && isClub(cl) ? genClause(p, cl.leagueId) : 0;
    }
  }
  // Sell-on clauses: when a player is sold, the club holding a sell-on gets its share of the fee.
  // Returns what the selling club keeps.
  function creditSale(p, sellerId, buyerId, fee) {
    let keep = fee;
    const so = p.sellOn;
    if (so && fee > 0 && S.clubs[so.clubId] && so.clubId !== buyerId && so.clubId !== sellerId) {
      const cut = niceRound((fee * so.pct) / 100);
      S.clubs[so.clubId].budget += cut; keep -= cut;
      const c = C();
      if (c && (sellerId === c.clubId || so.clubId === c.clubId)) news(`🧾 Sell-on clause: ${clubName(so.clubId)} receive ${so.pct}% of ${p.name}'s ${money(fee)} fee (${money(cut)}).`, so.clubId === c.clubId ? 'good' : 'info');
    }
    p.sellOn = null;
    if (S.clubs[sellerId] && sellerId !== 'FA') S.clubs[sellerId].budget += keep;
    return keep;
  }
  ...
  // Share of club revenue the board releases into your transfer budget (the rest pays wages, running costs and debts).
  const boardShare = () => { const b = C()?.board; return b ? clamp(0.1 + b.conf / 500, 0.1, 0.3) : 1; };
  function award(id, amount, note) {
    if (!id || !amount || !S.clubs[id]) return;
    const c = C();
    if (c && id === c.clubId) {
      const toBudget = niceRound(amount * boardShare());
      S.clubs[id].budget += toBudget;
      c.income = (c.income || 0) + amount;
      c.budgetIn = (c.budgetIn || 0) + toBudget;
      if (note) news(`💶 ${note}: ${money(amount)}${toBudget < amount ? ` (${money(toBudget)} to your transfer budget)` : ''}.`, 'good');
      return;
    }
  ...
  function startCareer(clubId, manager, opts = {}) {
    ...
    if (!pid) { S.career.board = newBoard(); setBoardGoals(); }
  ...
  function playMatch(comp, rd, ri, m, mi, withText, pend) {
    ...
    if (home && !rd.final) {
      ...
      const toBudget = niceRound(gate * boardShare());
      S.clubs[uid].budget += toBudget;
      c.income = (c.income || 0) + gate;
      c.budgetIn = (c.budgetIn || 0) + toBudget;
    }
    const [gf, ga] = home ? [m.hg, m.ag] : [m.ag, m.hg];
    boardAfterMatch(comp, gf, ga, m);
  ...
  function playDay(withText) {
    const c = C();
    if (c.seasonOver || c.retired || c.sacked) return null;
    ...
    { const wk = windowInfo().key; if (wk && wk.endsWith('J') && c.aiJan !== wk) { c.aiJan = wk; aiRenewKeys(); aiFillNeeds(0.35); } }
  ...
  function nextUserMatch() {
    const c = C();
    if (c.seasonOver || c.sacked) return null;
  ...
  function simUntilDay(target) {
    const c = C();
    let n = 0;
    while (!c.seasonOver && !c.sacked && c.dayIdx < target) { playDay(false); n++; }
    save();
    return n;
  }
  ...
  function wageReview(s, table) {
    ...
    if (c.board && c.board.conf >= 75) add(5, 'the board\'s trust in you');
    else if (c.board && c.board.conf < 30) add(-5, 'the board losing faith in you');
    const pct = clamp(reasons.reduce((a, r) => a + r.pct, 0), -25, 50);
    return { pct, reasons };
  }

  /* --- The board: objectives, confidence, money, and the sack --- */
  const newBoard = (conf = 60) => ({ conf, goals: [], long: null, asked: null });
  const confLabel = (x) => (x >= 80 ? 'Delighted' : x >= 62 ? 'Pleased' : x >= 45 ? 'Content' : x >= 30 ? 'Concerned' : x >= 15 ? 'Losing patience' : 'Your job is on the line');
  const expectedRank = (lid, id) => S.leagues[lid].clubIds.filter((x) => S.clubs[x]).sort((a, b) => clubRating(b) - clubRating(a)).indexOf(id) + 1;
  // Objectives for the new season (and a long-term target over the next few seasons).
  function setBoardGoals() {
    const c = C();
    if (isPlayerMode()) return;
    const b = c.board || (c.board = newBoard());
    const L = S.leagues[c.leagueId], n = L.clubIds.length, E = expectedRank(c.leagueId, c.clubId), tier = L.tier || 1;
    const goals = [];
    let target, text;
    ...
    if (eu) { const deep = eu.id === 'UCL' && E <= 2 ? 'QF' : 'R16'; goals.push({ type: 'euro', comp: eu.id, target: deep, text: `${eu.name}: reach the ${STAGE_NAME[deep]}` }); }
    b.goals = goals; b.season = c.season;
    if (!b.long || b.long.done || b.long.failed) {
      const by = c.season + 2;
  ...
  const STAGE_IDX = ['PO', 'R16', 'QF', 'SF', 'F'];
  function goalStatus(g) {
    const c = C(), uid = c.clubId;
    if (g.type === 'league') { const t = leagueTable(c.leagueId), pos = t.findIndex((r) => r.id === uid) + 1; return { ok: pos && pos <= g.target, live: `now ${ordinal(pos)}` }; }
    const comp = c.comps[g.comp];
    if (!comp) return { ok: false, live: '—' };
    if (g.type === 'cup') { const ok = comp.winner === uid || ['Semi-finals', 'Final'].includes(comp.out[uid]); return { ok, live: comp.winner === uid ? 'Won it' : comp.out[uid] ? `out: ${comp.out[uid]}` : 'still in' }; }
    const out = comp.out[uid], reached = comp.winner === uid ? 5 : out ? STAGE_IDX.findIndex((k) => STAGE_NAME[k] === out) : -1;
    return { ok: reached >= STAGE_IDX.indexOf(g.target), live: comp.winner === uid ? 'Won it' : out ? `out: ${out}` : 'still in' };
  }
  function boardAfterMatch(comp, gf, ga, m) {
    const c = C(), b = c.board;
    if (!b || isPlayerMode()) return;
    const big = comp.type !== 'league' ? 1.5 : 1;
    const before = b.conf;
    b.conf = clamp(b.conf + (gf > ga ? 0.7 : gf < ga ? -1.1 : -0.1) * big, 0, 100);
    if (before >= 30 && b.conf < 30) news('😠 The board is losing patience with recent results.', 'bad');
    const played = c.comps['L-' + c.leagueId]?.rounds.filter((r) => r.matches.every((x) => x.played)).length || 0;
    if (b.conf <= 6 && played >= 8) sackManager('a terrible run of results');
  }
  function lowerJobOffers() {
    const c = C(), cur = prestige(c.clubId);
    const clubs = Object.values(S.clubs).filter((cl) => isClub(cl) && cl.id !== c.clubId && cl.pids.length);
    let l = shuffle(clubs.filter((cl) => { const pr = prestige(cl.id); return pr >= cur - 16 && pr <= cur - 3; })).slice(0, 3);
    if (!l.length) l = shuffle(clubs.filter((cl) => prestige(cl.id) < cur)).slice(0, 3);
    return l.map((cl) => cl.id);
  }
  function sackManager(why) {
    const c = C();
    if (c.sacked || isPlayerMode()) return;
    c.sacked = { club: clubName(c.clubId), date: curDate(), why, offers: lowerJobOffers() };
    c.event = null; c.offers = [];
    news(`🔴 ${c.manager} has been sacked by ${clubName(c.clubId)} after ${why}.`, 'bad');
  }
  // End-of-season verdict on the objectives.
  function boardReview(s) {
    const c = C(), b = c.board, uid = c.clubId;
    if (!b || isPlayerMode()) return null;
    const lines = [];
    let d = 0;
    const add = (x, t) => { d += x; lines.push({ x: Math.round(x), t }); };
    for (const g of b.goals) {
      if (g.type === 'league') {
        const ok = s.pos <= g.target || (g.target === 3 && s.promoted);
        add(ok ? 7 + Math.min(6, (g.target - s.pos) * 2) : -(8 + Math.min(16, (s.pos - g.target) * 3)), `${g.text}: ${ok ? 'achieved' : 'missed'} (${ordinal(s.pos)})`);
      } else {
        const st = goalStatus(g);
        add(st.ok ? 5 : -5, `${g.text}: ${st.ok ? 'achieved' : 'missed'}`);
      }
    }
    for (const t of s.trophies) add(3, `won the ${t}`);
    const L = b.long;
    if (L && !L.done && !L.failed) {
      const n = (s.table || []).length || 20;
      const done = L.type === 'uclwin' ? s.trophies.includes(EURO.UCL.name) : L.type === 'title' ? s.trophies.includes(S.leagues[c.leagueId].name) : L.type === 'ucl' ? s.nextEuro === 'UCL' : L.type === 'promotion' ? !!s.promoted : s.pos <= Math.ceil(n / 2);
      if (done) { L.done = true; add(12, `long-term goal achieved: ${L.text}`); }
      else if (c.season >= L.by) { L.failed = true; add(-22, `long-term goal failed: ${L.text}`); }
    }
    if (s.relegated) add(-15, 'relegation');
    b.conf = clamp(b.conf + d, 0, 100);
    const res = { d: Math.round(d), conf: Math.round(b.conf), lines };
    if (b.conf < 15) sackManager(`a season that fell well short of the board's objectives`);
    else news(`📋 Board review: ${confLabel(b.conf)} (${Math.round(b.conf)}/100). ${lines.map((l) => l.t).join('; ')}.`, d >= 0 ? 'good' : 'bad');
    return res;
  }
  const askAmounts = () => { const base = niceRound(Math.max(2e6, (clubRating(C().clubId) - 60) * 1.2e6 * leagueFactor())); return [base, niceRound(base * 2.5), niceRound(base * 5)]; };
  function askBoard(k) {
    const c = C(), b = c.board, w = windowInfo(), key = w.key || 'closed-' + curDate().slice(0, 7);
    if (b.asked === key) return toast(w.key ? 'You have already asked the board during this transfer window.' : 'You have already asked the board this month.', 'bad');
    b.asked = key;
    const amt = askAmounts()[k];
    const chance = clamp((b.conf - 38) / 80 - k * 0.14, 0.02, 0.7);
    if (rand() < chance) {
      S.clubs[c.clubId].budget += amt;
      b.conf = clamp(b.conf - (3 + 3 * k), 0, 100);
      news(`💼 The board agree to give you an extra ${money(amt)}. They will expect results.`, 'good');
      toast(`The board released ${money(amt)}!`, 'good');
    } else {
      b.conf = clamp(b.conf - 2 - k, 0, 100);
      news(`💼 The board turned down your request for ${money(amt)}.${b.conf < 45 ? ' They want to see better results first.' : ' Try again in the next window.'}`, 'bad');
      toast('The board said no.', 'bad');
    }
    save();
  }
  function boardHtml() {
    const c = C(), b = c.board;
    if (!b) return '';
    const col = b.conf >= 62 ? 'var(--accent)' : b.conf >= 30 ? 'var(--gold)' : 'var(--red)';
    const goals = c.seasonOver ? [] : b.goals.map((g) => ({ g, st: goalStatus(g) }));
    return `<section class="card board-card">
      <h3>🏛️ The board <span class="ml-auto small" style="color:${col}">${confLabel(b.conf)}</span></h3>
      <div class="xpbar" title="Board confidence ${Math.round(b.conf)}/100"><i style="width:${Math.round(b.conf)}%;background:${col}"></i></div>
      <p class="muted small">Confidence ${Math.round(b.conf)}/100. The board puts <strong>${Math.round(boardShare() * 100)}%</strong> of club revenue into your transfer budget; the rest covers wages and running costs. Meet your objectives to earn more trust (and money). If confidence collapses, you will be sacked.</p>
      ${goals.length ? `<h4>This season</h4><ul class="plist">${goals.map(({ g, st }) => `<li>${st.ok ? '✅' : '⏳'} ${esc(g.text)}<span class="ml-auto small muted">${esc(st.live)}</span></li>`).join('')}</ul>` : ''}
      ${b.long ? `<h4>Long-term</h4><p class="small">${b.long.done ? '✅' : b.long.failed ? '❌' : '🎯'} ${esc(b.long.text)}</p>` : ''}
      <button class="btn sm" data-act="board-ask">💼 Ask the board for transfer funds</button>
    </section>`;
  }
  function sackedHtml() {
    const c = C(), k = c.sacked;
    return `<section class="card retire-card">
      <div class="gh-kicker">${fmtDate(k.date, true)}</div>
      <h2>🔴 You have been sacked</h2>
      <p>${esc(k.club)} have dismissed you after ${esc(k.why)}.</p>
      ${k.offers.length ? `<h4>Clubs willing to give you a chance</h4><ul class="offer-list loan-list">${k.offers.map((id) => { const cl = S.clubs[id]; return `<li>${crest(cl)}<div class="grow"><strong>${esc(cl.name)}</strong> ${ovrBadge(clubRating(id))} <span class="muted small">${esc(S.leagues[cl.leagueId].name)}</span><br><span class="small">Budget <span class="money">${money(cl.budget)}</span></span></div><button class="btn primary sm" data-act="take-job" data-id="${id}">Accept</button></li>`; }).join('')}</ul>` : '<p class="muted">No club has made an offer.</p>'}
      <div class="row gap mt"><button class="btn ghost danger" data-act="retire">Retire from management</button></div>
    </section>`;
  }
  /* --- AI clubs: fill real gaps in their team and keep their key players --- */
  function aiTransfer(best, buyer, fee) {
    const c = C(), from = S.clubs[best.clubId];
    buyer.budget -= fee;
    if (from.id !== 'FA') creditSale(best, from.id, buyer.id, fee);
    movePlayer(best, buyer.id);
    best.relc = genClause(best, buyer.leagueId);
    best.wage = wageFor(best);
    best.contract = c.season + randInt(2, 5);
    if (buyer.pids.length > 28) {
      const cut = buyer.pids.map((id) => S.players[id]).filter((x) => !x.loan && x.id !== best.id).sort((a, b) => a.ovr - b.ovr)[0];
      if (cut) movePlayer(cut, 'FA');
    }
    ...
    if (fee >= 40e6 || buyer.leagueId === c.leagueId || from.leagueId === c.leagueId) news(`🔁 ${best.name} joins ${buyer.name} from ${from.name}${fee ? ` for ${money(fee)}` : ' on a free'}.`, 'transfer');
  }
  function aiFillNeeds(frac = 1) {
    const c = C();
    const pool = Object.values(S.players).filter((p) => !p.loan && p.age <= 32 && (p.clubId === 'FA' || isClub(S.clubs[p.clubId])) && (isPlayerMode() ? p.id !== c.pid : p.clubId !== c.clubId));
    const byPos = {};
    for (const p of pool) (byPos[p.pos] = byPos[p.pos] || []).push(p);
    for (const k in byPos) byPos[k].sort((a, b) => b.ovr - a.ovr);
    let deals = 0;
    for (const club of shuffle(Object.values(S.clubs).filter((cl) => isClub(cl) && cl.pids.length && (isPlayerMode() || cl.id !== c.clubId)))) {
      if (rand() > frac || club.budget < 1e6) continue;
      if (!club.formation) club.formation = pickFormation(club);
      const slots = FORMATIONS[club.formation], xi = bestXI(club.pids, club.formation, { ignoreAvail: true });
      const effs = xi.map((id, k) => (id ? eff(S.players[id], slots[k][0]) : 30));
      const mean = avg(effs);
      let wi = -1, gap = 0;
      effs.forEach((e, k) => { const cur = xi[k] && S.players[xi[k]]; const aging = cur && cur.age >= 34 ? 4 : 0; const g = mean - e + aging + (slots[k][0] === 'GK' ? 2 : 0); if (g > gap) { gap = g; wi = k; } });
      if (wi < 0 || gap < 5) continue;
      const pos = slots[wi][0], we = effs[wi], bp = prestige(club.id);
      const cands = [];
      for (const ps of POSITIONS) { if (fit(ps, pos) < -1) continue; for (const p of byPos[ps] || []) { if (p.ovr < we + 3) break; if (p.ovr <= we + 14 && p.clubId !== club.id) cands.push(p); } }
      let best = null, bs = -1e9, fee = 0;
      for (const p of shuffle(cands).slice(0, 60)) {
        if (!S.players[p.id] || p.clubId === club.id) continue;
        if (p.clubId !== 'FA' && (prestige(p.clubId) > bp + 3 || (isUntouchable(p) && prestige(p.clubId) > bp - 4))) continue;
        const f = p.clubId === 'FA' ? niceRound(playerValue(p) * 0.3) : niceRound(clubValuation(p, club.id) * (0.95 + rand() * 0.15));
        if (f > club.budget * 0.9) continue;
        const sc = p.ovr + rand() * 2 - f / 4e7 - Math.max(0, p.age - 29);
        if (sc > bs) { bs = sc; best = p; fee = f; }
      }
      if (best) { aiTransfer(best, club, fee); deals++; }
    }
    return deals;
  }
  // AI clubs tie down their most important players before their contracts run out.
  function aiRenewKeys() {
    const c = C();
    for (const club of Object.values(S.clubs)) {
      if (!isClub(club) || (!isPlayerMode() && club.id === c.clubId)) continue;
      const key = club.pids.map((id) => S.players[id]).filter((p) => !p.loan).sort((a, b) => b.ovr - a.ovr).slice(0, 6);
      for (const p of key) {
        if (p.id === c.pid || p.contract > c.season || p.age > 32) continue;
        p.contract = c.season + randInt(2, 4); p.wage = Math.max(p.wage, wageFor(p));
        if (club.leagueId === c.leagueId && p.ovr >= 78) news(`✍️ ${p.name} signs a new contract at ${club.name} until ${contractLabel(p)}.`, 'transfer');
      }
    }
  }
  ...
  function takeJob(clubId) {
    ...
    const wasSacked = !!c.sacked;
    c.sacked = null;
    c.board = newBoard(wasSacked ? 50 : 58);
    if (!c.seasonOver) setBoardGoals();
  ...
  function startNextSeason() {
    ...
    cleanLineup();
    if (!isPlayerMode()) { if (!c.board) c.board = newBoard(); setBoardGoals(); }
  ...
  // What a sell-on clause of `pct`% is worth to the selling club now (young players are more likely to be sold on for more).
  const sellOnValue = (p, pct) => (pct ? niceRound((playerValue(p) * pct) / 100 * (p.age <= 21 ? 1.2 : p.age <= 25 ? 0.9 : p.age <= 29 ? 0.55 : 0.3)) : 0);
  function submitBid(pid, amount, swapPid, sellOn = 0) {
    ...
    const sv = (q ? swapValue(q, p.clubId) : 0) + sellOnValue(p, sellOn);
    t.bids++;
    t.sellOn = sellOn; t.counterSellOn = sellOn;
    if (q) {
      const total = amount + sv, pay = Math.max(0, val - sv);
      const worth = `They value ${q.name} at ${money(swapValue(q, p.clubId))}.`;
      if (total >= val) { t.agreed = amount; t.swap = q.id; return { status: 'accepted', msg: `${clubName(p.clubId)} accept the swap: ${q.name}${amount ? ` + ${money(amount)}` : ''}${sellOn ? ` (${sellOn}% sell-on)` : ''} for ${p.name}.` }; }
    ...
    const so = sellOnValue(p, sellOn), soTxt = sellOn ? ` with a ${sellOn}% sell-on clause` : '';
    if (amount + so >= val) { t.agreed = amount; t.swap = null; return { status: 'accepted', msg: `${p.clubId === 'FA' ? 'Signing fee agreed' : `${clubName(p.clubId)} accept your offer`} of ${money(amount)}${soTxt}.` }; }
    if (amount + so < val * 0.6) { t.blocked = true; return { status: 'ended', msg: `${clubName(p.clubId)} found that offer insulting and ended talks until the next window.` }; }
    if (t.bids >= 3) { t.blocked = true; return { status: 'ended', msg: `After three rejected bids, ${clubName(p.clubId)} have ended talks until the next window.` }; }
    if (amount + so >= val * 0.85) { t.counter = val - so; t.counterSwap = null; return { status: 'counter', msg: `${clubName(p.clubId)} reject the offer but would accept ${money(val - so)}${soTxt}.` }; }
  ...
  // A release clause the player can use to leave is worth a wage discount (the lower the clause, the bigger it is).
  const RELC_OPTS = [[0, 'No release clause', 0], [1.5, '1.5× his value', 0.1], [2, '2× his value', 0.06], [3, '3× his value', 0.03]];
  const relcDisc = (m) => (RELC_OPTS.find((o) => o[0] === m) || RELC_OPTS[0])[2];
  // Signing-on fee: a one-off payment is worth a bit more to him than the same money spread over his wages.
  const bonusWeekly = (bonus, years) => (bonus ? (bonus * 1.25) / (52 * years) : 0);
  function wageTalk(p, t, offer, years, ctx, bonus = 0, relc = 0) {
    ...
    const worth = offer * (1 + relcDisc(relc)) + bonusWeekly(bonus, years);
    const extras = `${bonus ? ` plus a ${money(bonus)} signing-on fee` : ''}${relc ? ` and a release clause of ${relc}× his value` : ''}`;
    w.bids++;
    if (worth >= floor) { w.agreed = { wage: offer, years, bonus, relc }; w.counter = 0; return { status: 'accepted', msg: `Agreed! ${p.name} accepts ${money(offer)} a week for ${years} year${years > 1 ? 's' : ''}${extras}.` }; }
    if (w.bids >= 3) { w.blocked = true; return { status: 'ended', msg: `${p.name}'s agent walks away after three offers that were too low.` }; }
    if (worth >= demand * 0.78) { w.counter = Math.max(offer, niceWage((demand - bonusWeekly(bonus, years)) / (1 + relcDisc(relc)))); w.counterYears = years; w.counterBonus = bonus; w.counterRelc = relc; return { status: 'counter', msg: `${p.name}'s agent wants ${money(w.counter)} a week on a ${years}-year deal${extras}. (${3 - w.bids} offer${3 - w.bids === 1 ? '' : 's'} left)` }; }
  ...
  // Paying a release clause: the club cannot refuse (the player still has to want to come).
  function payReleaseClause(pid) {
    const c = C(), p = S.players[pid], t = talk(pid);
    if (!p.relc) return { status: 'error', msg: 'He has no release clause.' };
    if (p.relc > S.clubs[c.clubId].budget) return { status: 'error', msg: `You need ${money(p.relc)} in your budget to pay his release clause.` };
    t.agreed = p.relc; t.swap = null; t.sellOn = 0; t.clause = true; t.blocked = false;
    return { status: 'accepted', msg: `You pay ${p.name}'s ${money(p.relc)} release clause. ${clubName(p.clubId)} cannot stop the move. Now agree personal terms.` };
  }
  ...
  function completeSigning(pid) {
    ...
    const bonus = terms.bonus || 0;
    if (me.budget < fee + bonus) return toast(`You need ${money(fee + bonus)} (fee plus signing-on fee) in your budget.`, 'bad');
    ...
    me.budget -= fee + bonus;
    if (from && from.id !== 'FA') creditSale(p, from.id, c.clubId, fee);
    if (q) {
    ...
    movePlayer(p, c.clubId);
    p.relc = terms.relc ? niceRound(playerValue(p) * terms.relc) : 0;
    p.sellOn = t.sellOn && from && from.id !== 'FA' ? { clubId: from.id, pct: t.sellOn } : null;
    if (q) cleanLineup();
    ...
    news(`✍️ Signed ${p.name} (${p.pos}, ${p.ovr}) from ${fromName}${q ? ` in a swap for ${q.name}${fee ? ` plus ${money(fee)}` : ''}` : ` for ${money(fee)}`} on ${money(wage)} a week until ${contractLabel(p)}${bonus ? `, with a ${money(bonus)} signing-on fee` : ''}${p.relc ? `. Release clause ${money(p.relc)}` : ''}${p.sellOn ? `. ${clubName(p.sellOn.clubId)} have a ${p.sellOn.pct}% sell-on clause` : ''}.`, 'good');
    save();
  ...
  function sellPlayer(pid, clubId, amount, quiet) {
    ...
    movePlayer(p, clubId);
    if (clubId !== 'FA') p.relc = genClause(p, buyer.leagueId);
  ...
  function endLoan(p, sold) {
    ...
    if (sold) {
      creditSale(p, from.id, club.id, sold); club.budget -= sold;
  ...
  function maybeIncomingOffer() {
    ...
    if (rand() < 0.2 && triggerClauseOnYou()) return;
  ...
  // A bigger club pays the release clause of one of your players: you cannot stop it.
  function triggerClauseOnYou() {
    const c = C(), me = S.clubs[c.clubId];
    if (me.pids.length <= MIN_SQUAD) return false;
    const cands = me.pids.map((id) => S.players[id]).filter((p) => p.relc > 0 && !p.loan && p.relc <= playerValue(p) * 2.2 && p.ovr >= clubRating(c.clubId) - 2);
    for (const p of shuffle(cands)) {
      const buyer = Object.values(S.clubs).find((cl) => isClub(cl) && cl.id !== c.clubId && cl.budget >= p.relc && prestige(cl.id) > prestige(c.clubId) + 1 && clubRating(cl.id) >= p.ovr - 3 && interest(p, cl.id).lvl >= 2);
      if (!buyer) continue;
      if (p.pos === 'GK' && me.pids.filter((id) => S.players[id].pos === 'GK').length <= 1) continue;
      buyer.budget -= p.relc;
      creditSale(p, c.clubId, buyer.id, p.relc);
      const fee = p.relc;
      movePlayer(p, buyer.id);
      p.wage = wageFor(p); p.contract = c.season + randInt(3, 5); p.relc = genClause(p, buyer.leagueId);
      c.offers = c.offers.filter((o) => o.pid !== p.id);
      cleanLineup();
      c.transfers.unshift({ season: c.season, date: curDate(), pid: p.id, name: p.name, dir: 'out', club: `${buyer.name} (release clause)`, fee });
      news(`💥 ${buyer.name} have paid ${p.name}'s ${money(fee)} release clause. He has left the club. Renew contracts without a clause (or with a higher one) to protect your stars.`, 'bad');
      return true;
    }
    return false;
  }
  ...
  function render() {
    ...
    app().innerHTML = `
      ...
      <main id="view" class="view">${c.retired ? retiredHtml() : c.sacked ? sackedHtml() : renderView()}</main>`;
    if (c.retired || c.sacked) return;
  ...
  function viewHome() {
    ...
    if (c.seasonOver) return seasonSummaryHtml() + boardHtml() + careerMovesHtml();
    const nm = nextUserMatch();
    ...
    return `
      <section class="card season-end">
        ...
        <div class="stats-row">
          ...
          <div class="big-stat"><span>Season income</span><strong class="money">${money(s.income)}</strong>${s.budgetIn != null && !isPlayerMode() ? `<small>${money(s.budgetIn)} went to your transfer budget</small>` : ''}</div>
          ${s.board ? `<div class="big-stat"><span>Board verdict</span><strong class="${s.board.d >= 0 ? 'money' : 'warn-t'}">${confLabel(s.board.conf)} (${s.board.d >= 0 ? '+' : ''}${s.board.d})</strong><small>${s.board.lines.map((l) => `${l.x >= 0 ? '+' : ''}${l.x} ${esc(l.t)}`).join(' · ')}</small></div>` : ''}
  ...
  function termsHtml(p, t, ctx, intro, prefix) {
    ...
    return `<div class="terms">
      ...
      <table class="tbl kv"><tbody>
        ...
        ${ctx === 'renew' ? `<tr><td>Release clause now</td><td class="money">${p.relc ? money(p.relc) : 'None'}</td></tr>` : ''}
      </tbody></table>
      ${w.msg ? `<div class="notice ${w.status === 'accepted' ? 'good' : w.status === 'counter' ? 'info' : ''}">${esc(w.msg)}</div>` : ''}
      ${w.agreed ? `<div class="row gap wrap mt"><button class="btn primary big" data-act="${prefix}-complete" data-id="${p.id}">${ctx === 'renew' ? 'Sign new contract' : 'Complete signing'}: ${money(w.agreed.wage)}/wk for ${w.agreed.years} yr${w.agreed.years > 1 ? 's' : ''}${w.agreed.bonus ? ` + ${money(w.agreed.bonus)} signing-on fee` : ''}</button></div>`
        : w.blocked ? '' : `<div class="row gap wrap mt">
          ...
          <label class="inline">Signing-on fee (€M) <input class="input xs" id="wage-bonus" type="number" min="0" step="0.5" value="${((w.lastBonus || 0) / 1e6).toFixed(1).replace(/\.0$/, '')}"></label>
          <label class="inline">Release clause <select class="input sm" id="wage-relc">${RELC_OPTS.map(([m, l, d]) => `<option value="${m}" ${(w.lastRelc || 0) === m ? 'selected' : ''}>${l}${d ? ` (−${Math.round(d * 100)}% wage)` : ''}</option>`).join('')}</select></label>
          <button class="btn primary" data-act="${prefix}-wage" data-id="${p.id}">Offer contract</button>
          ${w.counter ? `<button class="btn" data-act="${prefix}-wage-accept" data-id="${p.id}">Accept ${money(w.counter)}/wk${w.counterBonus ? ` + ${money(w.counterBonus)} fee` : ''}</button>` : ''}
        </div>
        <p class="muted small">Agents quietly accept a bit less than they ask for. Younger players want more for longer deals, and older players take less for security. A <strong>signing-on fee</strong> is paid once from your transfer budget and lowers the wage he needs. A <strong>release clause</strong> lowers his wage too, but any club that pays it can sign him. After 3 failed offers the talks end.</p>`}
    </div>`;
  }
  ...
  function wageOfferFromForm() {
    ...
    const bonus = Math.max(0, Math.round((parseFloat($('#wage-bonus')?.value) || 0) * 1e6));
    const relc = parseFloat($('#wage-relc')?.value) || 0;
    return { wage, years, bonus, relc };
  }
    ...
    openModal(`
      <div class="neg">
        ...
        <table class="tbl kv"><tbody>
          ...
          ${fa ? '' : `<tr><td>Release clause</td><td class="money">${p.relc ? money(p.relc) : '<span class="muted">None</span>'}</td></tr>`}
          ${p.sellOn ? `<tr><td>Sell-on clause</td><td class="small">${p.sellOn.pct}% to ${esc(clubName(p.sellOn.clubId))}</td></tr>` : ''}
        ...
        ${!blocker && !agreed && !t.blocked ? `
          <div class="row gap wrap mt">
            ...
            ${fa ? '' : `<label class="inline">Sell-on clause <select class="input sm" id="neg-sellon">${[0, 10, 15, 20, 30].map((x) => `<option value="${x}" ${(ui.neg.sellOn || 0) === x ? 'selected' : ''}>${x ? `${x}% of any future fee` : 'None'}</option>`).join('')}</select></label>`}
            <button class="btn primary" data-act="neg-bid" data-id="${pid}">Submit offer</button>
            ${!fa && p.relc ? `<button class="btn" data-act="neg-release" data-id="${pid}" ${p.relc > me.budget ? 'disabled' : ''} title="The club cannot refuse">💥 Pay release clause · ${money(p.relc)}</button>` : ''}
          ...
          <p class="muted small">Clubs accept offers at or above their valuation and counter offers that are close. Bids far below it end talks. You get 3 bids per player per window.${fa ? '' : ' <strong>Swap deals:</strong> offer one of your players (plus cash if needed). The club values him by his price, age and whether he is good enough for their team, and he must be willing to join them. <strong>Sell-on clause:</strong> promise the club a share of the fee if you sell him later, and they accept less now (worth most for young players). <strong>Release clause:</strong> pay it and the club cannot say no.'}</p>` : ''}
        ${!blocker && agreed ? termsHtml(p, t, 'sign', `${swapQ ? `Swap agreed: ${swapQ.name} (${swapQ.pos}, ${swapQ.ovr})${t.agreed ? ` + ${money(t.agreed)}` : ''} goes to ${clubName(p.clubId)}` : `${t.clause ? 'Release clause paid' : 'Fee agreed'}: ${fa ? `signing fee ${money(val)}` : money(t.agreed)}`}${t.sellOn ? ` with a ${t.sellOn}% sell-on clause` : ''}. Now agree personal terms.`, 'neg') : ''}
      </div>`);
  }
    ...
    openModal(`
      <div class="pmodal">
        ...
        <div class="stats-row">
          ...
          <div class="big-stat"><span>Value</span><strong class="money">${money(playerValue(p))}</strong></div>
          <div class="big-stat"><span>Wage</span><strong class="money">${money(p.wage)}/wk</strong></div>
          <div class="big-stat"><span>Release clause</span><strong class="money">${p.relc ? money(p.relc) : '—'}</strong>${p.sellOn ? `<small>${p.sellOn.pct}% sell-on to ${esc(clubName(p.sellOn.clubId))}</small>` : ''}</div>
  ...
  const ACTIONS = {
    ...
    'board-ask': () => {
      const [a, b2, c3] = askAmounts(), conf = C().board.conf;
      openModal(`<h2>💼 Ask the board for money</h2><p class="muted">The board are ${confLabel(conf).toLowerCase()} with you (${Math.round(conf)}/100). They are more generous when you are meeting your objectives, and asking for a lot is less likely to work. Whatever they say, it costs you a little of their patience, and you can only ask once per transfer window.</p>
        <div class="row gap wrap">${[a, b2, c3].map((v, i) => `<button class="btn ${i ? '' : 'primary'}" data-act="board-ask-do" data-id="${i}">${['Small top-up', 'Serious backing', 'Big statement'][i]} · ${money(v)}</button>`).join('')}</div>`);
    },
    'board-ask-do': (i) => { closeModal(); askBoard(+i); render(); },
    ...
    'neg-bid': (id) => {
      ...
      const sellOn = +($('#neg-sellon')?.value || 0);
      const r = submitBid(id, amount, swap, sellOn);
      ui.neg = { pid: id, msg: r.msg, status: r.status, last: amount, swap, sellOn };
      negModal(id);
    },
    'neg-release': (id) => { const r = payReleaseClause(id); if (r.status === 'error') return toast(r.msg, 'bad'); ui.neg = { pid: id, msg: r.msg, status: r.status }; negModal(id); },
    'neg-counter': (id) => { const t = talk(id); t.sellOn = t.counterSellOn || 0; if (t.counter > S.clubs[C().clubId].budget) return toast('Not enough transfer budget.', 'bad'); t.agreed = t.counter; t.swap = t.counterSwap || null; ui.neg = { pid: id, msg: t.swap ? 'Swap agreed. Now agree personal terms.' : `Fee agreed: ${money(t.agreed)}. Now agree personal terms.`, status: 'accepted' }; negModal(id); },
    'neg-sign': (id) => {
      const p = S.players[id];
    ...
    'ren-wage': (id) => { const { wage, years, bonus, relc } = wageOfferFromForm(); const t = renewTalk(id); const r = wageTalk(S.players[id], t, wage, years, 'renew', bonus, relc); Object.assign(t.w, { msg: r.msg, status: r.status, lastOffer: wage, lastYears: years, lastBonus: bonus, lastRelc: relc }); renewModal(id); },
    'ren-wage-accept': (id) => { const t = renewTalk(id); t.w.agreed = { wage: t.w.counter, years: t.w.counterYears || 3, bonus: t.w.counterBonus || 0, relc: t.w.counterRelc || 0 }; t.w.msg = 'Terms agreed.'; t.w.status = 'accepted'; renewModal(id); },
    'ren-complete': (id) => { if (completeRenewal(id)) { closeModal(); render(); } },
    'neg-wage': (id) => { const { wage, years, bonus, relc } = wageOfferFromForm(); const t = talk(id); const r = wageTalk(S.players[id], t, wage, years, 'sign', bonus, relc); Object.assign(t.w, { msg: r.msg, status: r.status, lastOffer: wage, lastYears: years, lastBonus: bonus, lastRelc: relc }); negModal(id); },
    'neg-wage-accept': (id) => { const t = talk(id); t.w.agreed = { wage: t.w.counter, years: t.w.counterYears || 3, bonus: t.w.counterBonus || 0, relc: t.w.counterRelc || 0 }; t.w.msg = 'Terms agreed.'; t.w.status = 'accepted'; negModal(id); },
  ...
  function upgradeState(st) {
    ...
    assignClauses(st);
    if (st.career && st.career.mode !== 'player' && !st.career.board) { st.career.board = newBoard(); if (!st.career.seasonOver) setBoardGoals(); }
  ...
  window.SM = { _pb: () => ui.playback, serialize, get state() { return S; }, get world() { return W; }, simulateMatch, buildSide, playDay, advanceToUserMatch, nextUserMatch, simUntilDay, startCareer, startNextSeason, importRecords, rowsToRecords, playerValue, leagueTable, phaseTable, submitBid, swapValue, payReleaseClause, triggerClauseOnYou, askBoard, aiFillNeeds, boardReview, setBoardGoals, wageTalk, creditSale, completeSigning, talk, interest, clubValuation, windowInfo, matchLineup, go, render };
})();
